        ...
        <?php foreach ($products as $prod): ?>
            <div class="bg-white rounded-xl shadow-sm p-4">
                <div class="flex gap-3">
                    <?php if ($prod['image']): ?>
                        <img src="../<?= h(UPLOAD_URL . $prod['image']) ?>" class="w-16 h-16 rounded-xl object-cover flex-shrink-0">
                    <?php else: ?>
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <button onclick="toggleActive(<?= $prod['id'] ?>, this)"
                                    class="px-2 py-0.5 rounded-full text-xs font-medium flex-shrink-0 <?= $prod['is_active'] ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500' ?>">
                                <?= $prod['is_active'] ? 'Actif' : 'Masqué' ?>
                            </button>
                        </div>
                        <p class="text-xs text-gray-400 mt-0.5"><?= h($prod['category_name'] ?? 'Sans catégorie') ?></p>
                        <p class="text-sm font-bold text-primary-700 mt-1">
                ...
                <div class="flex gap-2 mt-3 pt-3 border-t">
                    <a href="product-form.php?id=<?= $prod['id'] ?>" class="flex-1 text-center bg-primary-50 text-primary-700 py-2 rounded-lg text-sm font-medium hover:bg-primary-100 transition">Modifier</a>
                    <form method="POST" action="product-delete.php" class="flex-1" onsubmit="return confirm('Supprimer ce produit ?')">
                        <input type="hidden" name="csrf_token" value="<?= csrf_token() ?>">
                        <input type="hidden" name="id" value="<?= $prod['id'] ?>">
                        <button type="submit" class="w-full bg-red-50 text-red-600 py-2 rounded-lg text-sm font-medium hover:bg-red-100 transition">Supprimer</button>
                    </form>
                </div>
            </div>
        <?php endforeach; ?>
